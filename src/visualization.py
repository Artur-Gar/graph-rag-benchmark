...
def plot_evaluation_dashboard(
    ...
    """Create and save the combined synthetic and real data evaluation dashboard """
    ...
    im = axes[0, 0].imshow(heat.values, cmap="RdYlGn", aspect="auto", vmin=-50, vmax=50)
    axes[0, 0].set_title("Synthetic: Path Fidelity Gain (RAG - Baseline)")
    axes[0, 0].set_xticks(range(len(heat.columns)))
    axes[0, 0].set_xticklabels(heat.columns, rotation=25, ha="right")
    axes[0, 0].set_yticks(range(len(heat.index)))
    axes[0, 0].set_yticklabels([str(x) for x in heat.index])
    axes[0, 0].set_xlabel("Retrieval Strategy")
    axes[0, 0].set_ylabel("Graph Size N")
    for i in range(heat.shape[0]):
        for j in range(heat.shape[1]):
            axes[0, 0].text(j, i, f"{heat.values[i, j]:.0f}", ha="center", va="center", color="black")
    ...
    axes[0, 1].plot(size_avg["N"], size_avg["baseline_path_pct"], marker="o", linewidth=2, label="Baseline Path")
    axes[0, 1].plot(size_avg["N"], size_avg["rag_path_pct"], marker="o", linewidth=2, label="RAG Path")
    axes[0, 1].plot(
        size_avg["N"],
    ...
    axes[0, 1].set_title("Synthetic: Performance vs Graph Size (Avg over strategies)")
    ...
    real_path_gain.plot(kind="bar", ax=axes[1, 0])
    axes[1, 0].set_title("Real Data: Path Fidelity Gain (RAG - Baseline)")
    axes[1, 0].set_xlabel("Dataset")
    axes[1, 0].set_ylabel("Gain (percentage points)")
    ...
    for _, row in tradeoff.iterrows():
        axes[1, 1].scatter(row["conn_delta"], row["path_delta"], s=140)
        axes[1, 1].annotate(
            row["strategy"],
    ...
    axes[1, 1].set_title("Real Data: Strategy Trade-off")
    axes[1, 1].set_xlabel("Avg Connectivity Gain (pp)")
    axes[1, 1].set_ylabel("Avg Path Gain (pp)")
